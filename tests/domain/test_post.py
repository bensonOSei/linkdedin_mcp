...
def test_creates_with_draft_status(sample_post: Post) -> None:
    assert sample_post.status == PostStatus.DRAFT


def test_creates_with_generated_id(sample_post: Post) -> None:
    assert sample_post.id is not None
    assert len(sample_post.id) > 0


def test_creates_with_empty_hashtags(sample_post: Post) -> None:
    assert sample_post.hashtags == []


def test_creates_with_no_engagement_score(sample_post: Post) -> None:
    assert sample_post.engagement_score is None


def test_schedule_from_draft(sample_post: Post) -> None:
...
def test_schedule_from_scheduled_raises(sample_post: Post) -> None:
    sample_post.schedule(datetime.now(timezone.utc))
    with pytest.raises(ValueError, match="Cannot schedule"):
        sample_post.schedule(datetime.now(timezone.utc))


def test_schedule_from_published_raises(sample_post: Post) -> None:
    sample_post.schedule(datetime.now(timezone.utc))
    sample_post.publish()
    with pytest.raises(ValueError, match="Cannot schedule"):
        sample_post.schedule(datetime.now(timezone.utc))
...
def test_publish_from_scheduled(sample_post: Post) -> None:
    sample_post.schedule(datetime.now(timezone.utc))
    sample_post.publish()
    assert sample_post.status == PostStatus.PUBLISHED


def test_publish_from_draft(sample_post: Post) -> None:
    sample_post.publish()
    assert sample_post.status == PostStatus.PUBLISHED


def test_publish_already_published_raises(sample_post: Post) -> None:
    sample_post.publish()
    with pytest.raises(ValueError, match="already published"):
        sample_post.publish()


def test_update_content_on_draft(sample_post: Post) -> None:
...
def test_update_content_on_published_raises(sample_post: Post) -> None:
    sample_post.schedule(datetime.now(timezone.utc))
    sample_post.publish()
...
def test_add_hashtags(sample_post: Post) -> None:
    tags = [
...
def test_set_engagement_score(sample_post: Post) -> None:
